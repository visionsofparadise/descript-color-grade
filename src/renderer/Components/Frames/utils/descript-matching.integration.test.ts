...
import { buildUniforms, type DescriptGradeAdjustments } from "./grade-uniforms";
import { createGradeProgram, createTexture, drawGrade, readPixelsToImageData, uploadTexture } from "./grade-webgl";
...
function gradeViaShader(input: ImageData, adjustments: DescriptGradeAdjustments): ImageData {
	const { gl, program, texture } = ensureHarness(input.width, input.height);

	uploadTexture(gl, texture, input);
	drawGrade(gl, program, texture, buildUniforms(adjustments, { colorModel: "legacy" }));
...
function makeImageData(rgbs: Array<[number, number, number]>): ImageData {
...
function extractRgb(image: ImageData): Array<[number, number, number]> {
...
const contrastStrengthKeys = ["-100", "-50", "-25", "-10", "+25", "+50", "+100"] as const;

describe("DescriptGrade — Contrast", () => {
	for (const key of contrastStrengthKeys) {
		it(`${key}`, () => {
			const unit = Number(key) / 100;
			assertPatches(`contrast ${key}`, { contrast: unit }, golden.properties.contrast[key], TOLERANCE);
		});
	}
});
...
describe("DescriptGrade — Exposure", () => {
	for (const key of fourStrengthKeys) {
		it(`${key}`, () => {
			const unit = Number(key) / 100;
			assertPatches(`exposure ${key}`, { exposure: unit }, golden.properties.exposure[key], TOLERANCE);
		});
	}
});
...
describe("DescriptGrade — Saturation", () => {
	for (const key of fourStrengthKeys) {
		it(`${key}`, () => {
			const unit = Number(key) / 100;
			assertPatches(`saturation ${key}`, { saturation: unit }, golden.properties.saturation[key], TOLERANCE);
		});
	}
});
...
describe("DescriptGrade — Temperature", () => {
	for (const key of fourStrengthKeys) {
		it(`${key}`, () => {
			const unit = Number(key) / 100;
			assertPatches(`temperature ${key}`, { temperature: unit }, golden.properties.temperature[key], TOLERANCE);
		});
	}
});
...
describe("DescriptGrade — Tint", () => {
	for (const key of fourStrengthKeys) {
		it(`${key}`, () => {
			const unit = Number(key) / 100;
			assertPatches(`tint ${key}`, { tint: unit }, golden.properties.tint[key], TOLERANCE);
		});
	}
});
